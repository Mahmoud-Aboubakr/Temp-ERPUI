...
export class ListComponent {
  displayedColumns: string[] = ['ServiceName', 'ItemCode', 'CostPrice', 'CostPriceCurrency', 'Controls']; // Add more columns as needed
  searchTerm: string = '';
  paginationResponseModel: PaginationResponseModel<ItemModel[]> = {
    currentPage:0,
    errorMessage: '',
    lang:'',
    message:'',
  ...
  dataSource = new MatTableDataSource<ItemModel>(this.paginationResponseModel.data);
  ...
  async getData(){ 
    ...
    await lastValueFrom(this._commonCrudService.getAllWithSearch("Items/GetItems", this.paginationParamWithSearch, this.paginationResponseModel)).then(res => {
  ...
  async updateItem(id){ 
    this.router.navigate(['inventory/setup/items/update/' + id]);
  }
  async deleteItem(id){ 
    this.router.navigate(['inventory/setup/items/delete/' + id]);
  }
  async addItem(){ 
    this.router.navigate(['inventory/setup/items/create']);
  }
