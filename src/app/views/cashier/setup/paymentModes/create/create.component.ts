...
@Component({
  selector: 'app-payment-modes-create',
...
export class CreateComponent implements OnInit {
  ...
  async save(){ 
    if(this.model.valid){
    let addModel = new PaymentModesModel(); 
    addModel.PaymentModeName = this.model.controls['paymentModeName'].value; 
    addModel.PaymentModesTypeId = this.model.controls['paymentModeTypeId'].value; 
    addModel.IsActive = this.model.controls['isActive'].value == '' ? false : true; 
    await lastValueFrom(this._commonCrudService.post("PaymentModes/AddPaymentModes", addModel, this.responseModel)).then(res => {
