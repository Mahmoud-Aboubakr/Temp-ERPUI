...
export class ListComponent {
  displayedColumns: string[] = ['PaymentModeName', 'PaymentModeType', 'IsActive', 'Controls']; // Add more columns as needed
  searchTerm: string = '';
  paginationResponseModel: PaginationResponseModel<PaymentModesModel[]> = {
    currentPage:0,
    errorMessage: '',
    lang:'',
    message:'',
  ...
  dataSource = new MatTableDataSource<PaymentModesModel>(this.paginationResponseModel.data);
  ...
  async getData(){ 
    await lastValueFrom(this._commonCrudService.getAllWithSearch("PaymentModes/GetPaymentModes", this.paginationParamWithSearch, this.paginationResponseModel)).then(res => {
  ...
  async updatePaymentMode(id){ 
    this.router.navigate(['cashier/setup/paymentModes/update/' + id]);
  }
  async deletePaymentMode(id){ 
    this.router.navigate(['cashier/setup/paymentModes/delete/' + id]);
  }
  async addPaymentMode(){ 
    this.router.navigate(['cashier/setup/paymentModes/create']);
  }
