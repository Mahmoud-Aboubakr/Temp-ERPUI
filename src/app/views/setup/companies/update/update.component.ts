...
export class UpdateComponent implements OnInit {
  ...
  responseModel: ResponseModel<CompanyModel[]> = {
    message: '',
    statusCode: 0,
  ...
  async getData(id){ 
    await lastValueFrom(this._commonCrudService.get("Companies/" + id, this.responseModel))
    .then(res => {
      ...
      if(res.statusCode == 200){
          ...
          this.router.navigate(['setup/companies']);
      }
    }); 
  }
  ...
  async update(){ 
    if(this.model.valid){
      let updatedModel = new CompanyModel(); 
      ...
      await lastValueFrom(this._commonCrudService.update("Companies/" + this.Id, updatedModel, this.responseModel)
      ) 
      .then(res => {
        this.responseModel = res;
