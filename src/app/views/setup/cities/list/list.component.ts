...
export class ListComponent implements OnInit {
  ...
  paginationResponseModel: PaginationResponseModel<CityModel[]> = {
    currentPage:0,
    errorMessage: '',
    lang:'',
    message:'',
  ...
  dataSource = new MatTableDataSource<CityModel>(this.paginationResponseModel.data);
  ...
  async getData(){ 
    await lastValueFrom(this._commonCrudService.getAll("Cities/GetCities", this.paginationParam, this.paginationResponseModel)).then(res => {
  ...
  async add(){ 
    this.router.navigate(['setup/cities/create']);
  }

  async update(id){ 
    this.router.navigate(['setup/cities/update/' + id]);
  }

  async delete(id){ 
    this.router.navigate(['setup/cities/delete/' + id]);
  }
