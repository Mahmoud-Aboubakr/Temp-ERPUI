...
export class ListComponent implements OnInit {
  ...
  dataSource = new MatTableDataSource<CountryModel>(this.paginationResponseModel.data);
  ...
  async getData(){ 
    await lastValueFrom(this._commonCrudService.getAll("Countries/GetCountries", this.paginationParam, this.paginationResponseModel)).then(res => {
  ...
  async add(){ 
    this.router.navigate(['setup/countries/create']);
  }

  async update(id){ 
    this.router.navigate(['setup/countries/update/' + id]);
  }

  async delete(id){ 
    this.router.navigate(['setup/countries/delete/' + id]);
  }
