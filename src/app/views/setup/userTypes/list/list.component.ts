...
export class ListComponent {
  displayedColumns: string[] = ['typeName', 'descNameAr', 'descNameEn','fullDesc', 'Controls']; // Add more columns as needed
  paginationResponseModel: PaginationResponseModel<userTypeModel[]> = {
    currentPage:0,
    errorMessage: '',
    lang:'',
    message:'',
  ...
  dataSource = new MatTableDataSource<userTypeModel>(this.paginationResponseModel.data);
  ...
  async getData(){ 
    await lastValueFrom(this._commonCrudService.getAll("UserType/GetUserTypes", this.paginationParam, this.paginationResponseModel)).then(res => {
  ...
  async updateUserType(id){ 
    this.router.navigate(['setup/userTypes/update/' + id]);
  }
  async deleteUserType(id){ 
    this.router.navigate(['setup/userTypes/delete/' + id]);
  }
  async addNew(){ 
    this.router.navigate(['setup/userTypes/create']);
  }
